...
public class KGFGUIUtilityEditor
{
	...
	[MenuItem ("Edit/KGF/skins/editor/enable KGFSkins in editor")]
	public static void EnableKGFSkinsInEditor()
	{
		KGFGUIUtility.SetEnableKGFSkinsInEdior(true);
	}
	
	[MenuItem ("Edit/KGF/skins/editor/disable KGFSkins in editor")]
	public static void DisableKGFSkinsInEditor()
	{
		KGFGUIUtility.SetEnableKGFSkinsInEdior(false);
	}
	
	
	public static void RenderKGFInspector(KGFEditor theEditor)
	{
		RenderKGFInspector(theEditor,theEditor.target.GetType());
	}
	
	public static void RenderKGFInspector(KGFEditor theEditor, Type theType)
	{
	...
	/// </param>
	public static void RenderKGFInspector(KGFEditor theEditor, Type theType, System.Action theHandler)
	{
//		// use fixed skin
...
//					DrawCustomInspectorReflection(theEditor.target,theEditor.target,0);
	...
	public static object DrawField(UnityEngine.Object theTarget, Type theFieldType, string theName, object theValue, out bool theFoundType)
	{
		...
		} else
		{
			...
			else if (typeof(bool).IsAssignableFrom(theFieldType))
			{
				return ChangeCheck<bool>(theTarget,EditorGUILayout.Toggle(theName,(bool)theValue),(bool)theValue);
			}
		}
		theFoundType = false;
		return theValue;
...
//		KGFGUIUtility.EndVerticalBox();
	...
	public static void RenderWarning(string theWarning)
	{
		...
		{
			GUILayout.Label(new GUIContent(theWarning, itsIconWarning),GetMultiLineStyle(),GUILayout.ExpandWidth(true));
		}
		KGFGUIUtility.EndHorizontalBox();
		SetColorDefault();
	}
