...
public class MegaFFDEditor : MegaModifierEditor
{
	...
	{
		return Handles.FreeMoveHandle(pos, size, Vector3.zero, Handles.CircleHandleCap);
	}
		...
		if ( ffd.DisplayGizmo )
		{
			...
			if ( context && context.DrawGizmos )
			{
				...
				for ( int i = 0; i < pc; i++ )
				{
					...
					pm = Handles.FreeMoveHandle(p, Quaternion.identity, ffd.KnotSize * 0.1f, Vector3.zero, Handles.CircleHandleCap);
				...
				for ( int z = 0; z < gs; z++ )
				{
					for ( int y = 0; y < gs; y++ )
					{
						for ( int x = 0; x < gs; x++ )
						{
							...
							{
								var fmh_140_42_638176027079164677 = Quaternion.identity; ttp = Handles.FreeMoveHandle(tp, ffd.KnotSize * 0.1f, Vector3.zero, Handles.CircleHandleCap);
	...
	public static Vector3 PositionHandle(Vector3 position, Quaternion rotation, float size, float alpha)
	{
		...
		col.a = alpha;
		Handles.color = col;    //Color.red;	//Handles..xAxisColor;
		position = Handles.Slider(position, rotation * Vector3.right, handlesize, Handles.RectangleHandleCap, 0.0f); //SnapSettings.move.x);
		col = Color.green;
		col.a = alpha;
		Handles.color = col;    //Color.green;	//Handles.yAxisColor;
		position = Handles.Slider(position, rotation * Vector3.up, handlesize, Handles.RectangleHandleCap, 0.0f);    //SnapSettings.move.y);

		col = Color.blue;
		col.a = alpha;

		Handles.color = col;    //Color.blue;	//Handles.zAxisColor;
		position = Handles.Slider(position, rotation * Vector3.forward, handlesize, Handles.RectangleHandleCap, 0.0f);   //SnapSettings.move.z);

		col = Color.yellow;
		col.a = alpha;

		Handles.color = col;    //Color.yellow;	//Handles.centerColor;
		position = Handles.FreeMoveHandle(position, handlesize * 0.15f, Vector3.zero, Handles.RectangleHandleCap);
		Handles.color = color;
		return position;
	}
#endif
